...
void loop() {
  ...
  if (Serial.available() > 0) {
    String forceStr = Serial.readStringUntil('\n');
    if (!isRunning && forceStr.equals("START")) {
      isRunning = true;
    } else if (isRunning && forceStr.equals("STOP")) {
      isRunning = false;
      force = 0;
    } else if (isRunning){
       force = forceStr.toFloat();
       Serial.println(xh);
    } else {
      force = 0;
    }
    ...
    if(abs(force) > 2) {
      force = force > 0 ? force - 0.5 : force + 0.5;
    }
